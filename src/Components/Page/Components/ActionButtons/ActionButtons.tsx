import { Button, Card, Heading, Icon, Pane, SymbolCircleIcon, Text, Tooltip } from 'evergreen-ui'
...
export function ActionButtons({combatant1}: Props) {
  ...
  const staminaMarkup = staminaArray.map((_, i) => (
    <Pane width={32} height={32} borderRadius={16} margin={8} border='default' borderWidth='thin' background={getStaminaColor(i)} display='flex' justifyContent='center' alignItems='center'>
  ...
  function updateActions(e:any) {
    ...
    const target = e.target.nodeName === "SPAN" ? e.target : e.target.parentNode.parentNode.parentNode.lastChild;
    ...
    <Card display="flex" flexDirection="column" background='tint2' padding={16} alignItems="center">
      ...
      <Pane justifyContent="center">
        <Tooltip content={ACTIONS.attack.tip}>
          <Button margin={8} intent="success" onClick={(e:any) => updateActions(e)} disabled={chosenActions.length >= stamina}>
          <Icon icon={SymbolCircleIcon} onClick={(e:any) => updateActions(e)} />
        ...
        <Tooltip content={ACTIONS.special.tip}>
          <Button margin={8} intent="success"  onClick={(e:any) => updateActions(e)} disabled={chosenActions.length >= stamina-1}>
            <Icon icon={SymbolCircleIcon} onClick={(e:any) => updateActions(e)} />
            <Icon icon={SymbolCircleIcon} onClick={(e:any) => updateActions(e)} />
      ...
      <Pane margin={8} />
      <Pane display="flex" justifyContent="center">
        <Button appearance='primary' onClick={() => console.log(chosenActions)} disabled={chosenActions.length !== stamina}>Confirm</Button>
      <Pane margin={8} />
        <Button appearance='destructive' onClick={() => setChosenActions([])}>Clear</Button>
      </Pane>
    </Card>
  )
}
